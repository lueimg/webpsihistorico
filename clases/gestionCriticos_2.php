...
class gestionCriticos{
	...
	SELECT * FROM(
		SELECT * FROM(
					SELECT c.id,a.averia,id_atc,tipo_actividad,a.nombre_cliente 'nombre',a.fecha_registro 'fecha_reg',a.quiebre 'quiebres',emp.nombre COLLATE utf8_general_ci empresa,a.telefono_codclientecms 'telefono_cliente_critico',
								...
								(SELECT fecha_cambio FROM webpsi_coc.vistaAveriasCriticosFinal WHERE averia=a.averia) fecha_cambio, 
								(SELECT horas_cambio_vs_actual FROM webpsi_coc.vistaAveriasCriticosFinal WHERE averia=a.averia) horas_cambio 
	...
	SELECT * FROM(
		SELECT * FROM(
					...
					(CASE tmp.DATA17 WHEN '' THEN '' ELSE tmp.DATA17 END) 'existe',a.tipo_actuacion AS 'actuacion', '' as 'fecha_cambio','' as 'horas_cambio'
								FROM webpsi_criticos.gestion_criticos c INNER JOIN webpsi_criticos.gestion_movimientos mov ON c.id=mov.id_gestion INNER JOIN
	...
	SELECT * FROM(
		SELECT * FROM(
					...
					(SELECT fecha_cambio FROM webpsi_coc.vistaAveriasCriticosFinal WHERE averia=webpsi_coc.averias_criticos_final.averia) fecha_cambio, 
					(SELECT horas_cambio_vs_actual FROM webpsi_coc.vistaAveriasCriticosFinal WHERE averia=webpsi_coc.averias_criticos_final.averia) horas_cambio 
