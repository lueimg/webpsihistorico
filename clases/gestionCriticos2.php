...
class gestionCriticos{
	...
	SELECT * FROM(
		SELECT * FROM(
					SELECT c.id,a.averia,id_atc,tipo_actividad,a.nombre_cliente 'nombre',a.fecha_registro 'fecha_reg',a.quiebre 'quiebres',emp.nombre COLLATE utf8_general_ci empresa,a.telefono_codclientecms 'telefono_cliente_critico',
								...
								(SELECT ROUND((TIMESTAMPDIFF(SECOND,(SELECT fecha_cambio FROM webpsi_coc.`averias_criticos_final_historico` WHERE averia=a.averia),NOW()) / 3600),2)) AS `horas_cambio`,if(IFNULL(a.wu_nagendas,'')='',0,a.wu_nagendas) wu_nagendas
	...
	SELECT * FROM(
		SELECT * FROM(
					...
					(CASE tmp.DATA17 WHEN '' THEN '' ELSE tmp.DATA17 END) 'existe',a.tipo_actuacion AS 'actuacion', '' as 'fecha_cambio','' as 'horas_cambio',if(IFNULL(a.wu_nagendas,'')='',0,a.wu_nagendas) wu_nagendas
								FROM webpsi_criticos.gestion_criticos c INNER JOIN webpsi_criticos.gestion_movimientos mov ON c.id=mov.id_gestion INNER JOIN
								...
								webpsi_criticos.submotivos s ON c.id_submotivo=s.id INNER JOIN 
								webpsi_criticos.estados e ON c.id_estado=e.id LEFT JOIN
								schedulle_sistemas.tmp_gaudi_total tmp ON tmp.DATA17=a.codigo_req
								INNER JOIN webpsi_criticos.empresa emp ON emp.id=mov.id_empresa
								where a.quiebre<>'F_PL'
								AND mov.fecha_movimiento=(SELECT MAX(mov2.fecha_movimiento) FROM webpsi_criticos.gestion_movimientos mov2
	...
	SELECT * FROM(
		SELECT * FROM(
					...
					(SELECT fecha_cambio FROM webpsi_coc.averias_criticos_final_historico WHERE averia=webpsi_coc.averias_criticos_final.averia) fecha_cambio,
					(SELECT ROUND((TIMESTAMPDIFF(SECOND,(SELECT fecha_cambio FROM webpsi_coc.averias_criticos_final_historico WHERE averia=webpsi_coc.averias_criticos_final.averia),NOW()) / 3600),2)) AS `horas_cambio`,if(IFNULL(wu_nagendas,'')='',0,wu_nagendas) wu_nagendas
	...
	SELECT * FROM(
		SELECT * FROM(
					...
					fecha_data_fuente,telefono_codclientecms,rango_dias,sms1,
					sms2,area2,'' AS 'fecha_creacion',microzona,'' as 'tecnico', 0 nmov,'' as 'existe',tipo_actuacion as 'actuacion', '' as 'fecha_cambio','' as 'horas_cambio',if(IFNULL(wu_nagendas,'')='',0,wu_nagendas) wu_nagendas
	...
	UNION ALL
		SELECT * FROM(
					SELECT c.id,a.averia,id_atc,tipo_actividad,a.nombre_cliente 'nombre',a.fecha_registro 'fecha_reg',a.quiebre 'quiebres',a.eecc_final 'empresa',a.telefono_codclientecms 'telefono_cliente_critico',
								...
								a.fecha_data_fuente,a.telefono_codclientecms,a.rango_dias,a.sms1,
								a.sms2,a.area2,c.fecha_creacion,a.microzona,mov.tecnico, c.nmov,
								(case aver.averia when '' then '' ELSE aver.averia END) 'existe','' as 'actuacion', '' as 'fecha_cambio','' as 'horas_cambio',if(IFNULL(a.wu_nagendas,'')='',0,a.wu_nagendas) wu_nagendas
								FROM webpsi_criticos.gestion_criticos c inner join webpsi_criticos.gestion_movimientos mov on c.id=mov.id_gestion inner join
	...
	SELECT * FROM(
		...
		SELECT * FROM(
			SELECT * FROM(
					...
					fecha_data_fuente,a.telefono_codclientecms,a.rango_dias,a.sms1,a.sms2,
					a.area2,c.fecha_creacion AS 'fecha_creacion',a.microzona,mov.tecnico, c.nmov,
					(CASE tmp.DATA17 WHEN '' THEN '' ELSE tmp.DATA17 END) 'existe',a.tipo_actuacion AS 'actuacion',
					'' as 'fecha_cambio','' as 'horas_cambio',if(IFNULL(a.wu_nagendas,'')='',0,a.wu_nagendas) wu_nagendas
								FROM webpsi_criticos.gestion_criticos c INNER JOIN webpsi_criticos.gestion_movimientos mov ON c.id=mov.id_gestion INNER JOIN
								webpsi_criticos.gestion_provision a ON c.id=a.id_gestion INNER JOIN 
								webpsi_criticos.horarios h ON c.id_horario=h.id INNER JOIN 
								webpsi_criticos.motivos m ON c.id_motivo=m.id INNER JOIN 
		...
		SELECT * FROM(
			SELECT * FROM(
									...
									'' as 'flag_tecnico',tipo_averia,horas_averia,fecha_registro,ciudad,averia 'codigo_averia',inscripcion,fono1,telefono,mdf,observacion_102,segmento,
													...
													fecha_data_fuente,telefono_codclientecms,rango_dias,sms1,
													sms2,area2,'' AS 'fecha_creacion',microzona,'' as 'tecnico', 0 nmov,'' as 'existe','' as 'actuacion',
													fecha_cambio,horas_cambio_vs_actual as 'horas_cambio',if(IFNULL(wu_nagendas,'')='',0,wu_nagendas) wu_nagendas
													FROM webpsi_coc.vistaAveriasCriticosFinal
													WHERE averia NOT IN (SELECT distinct averia FROM webpsi_criticos.gestion_averia) $filtro_Averias
													and quiebre<>'F_PL' order by fecha_registro asc
			)AS m2
			UNION ALL
		...
		SELECT * FROM(
			SELECT * FROM(
					...
					'' as 'fecha_cambio','' as 'horas_cambio',if(IFNULL(wu_nagendas,'')='',0,wu_nagendas) wu_nagendas
											FROM webpsi_coc.tmp_provision
											WHERE codigo_req NOT IN (SELECT codigo_req FROM webpsi_criticos.gestion_provision) $filtro_Averias
											and quiebre<>'F_PL' order by fecha_registro asc
			)AS m3 
		...
		SELECT * FROM(
					...
					'' as 'existe','' as 'actuacion','' as 'fecha_cambio','' as 'horas_cambio',if(IFNULL(a.wu_nagendas,'')='',0,a.wu_nagendas) wu_nagendas
