...
class gestionAveria{
	...
	function updateEmpresa($cnx,$vempresa,$codigos){
		...
		try{
			...
			if($vempresa!=""){
				...
				$cad = "update webpsi_criticos.`gestion_averia` set eecc_final='$vempresa' where id_gestion in($codigos)";//sin contrarta o zonal
				//echo $cad;
				$res = $cnx->exec($cad);
			}else{
	        	$result["estado"] = FALSE;
			...
			if($vempresa!=""){
	        	...
	        	$gestMovimiento->updateEmpresaMovimientos($cnx,$codigos,$id_empresa);
	        }else{
	        	$result["estado"] = FALSE;
	    		$result["msg"] = "Seleccione una empresa";
	    		return $result;
