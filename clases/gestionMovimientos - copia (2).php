...
if($reporte=="act"){
       ...
       $sql = "SELECT * FROM(
                          SELECT * FROM( 
                                          SELECT cri.id_atc,a.eecc_final,
                                                             (CASE c.fecha_movimiento WHEN (SELECT MAX(mov2.fecha_movimiento) FROM webpsi_criticos.gestion_movimientos mov2
                                                                                         WHERE mov2.id_gestion=c.id_gestion) THEN 'X' ELSE '' END) 'ultimo_movimiento',tipo_actividad,a.averia,a.codmotivo_req_catv,a.quiebre,nombre_cliente_critico,telefono_cliente_critico,
                                          celular_cliente_critico,c.observacion, c.fecha_agenda,horario,d.dias, e.nombre,z.zonal,m.motivo,s.submotivo,es.estado,tecnico,
                                          a.fecha_registro,fecha_creacion, fecha_movimiento,t.usuario,es.id AS 'e_id',c.fecha_consolidacion,tipo_averia,averia_m1,
                                          l.penalizable,
                                          (SELECT fecha_cambio FROM webpsi_coc.`averias_criticos_final_historico` ach WHERE ach.averia=a.`averia` ) AS fecha_cambio
                                          FROM
                                          webpsi_criticos.gestion_averia a INNER JOIN webpsi_criticos.gestion_criticos cri ON a.id_gestion=cri.id INNER JOIN 
                                          webpsi_criticos.gestion_movimientos c ON c.id_gestion=cri.id INNER JOIN 
                                          webpsi_criticos.empresa e ON e.id=c.id_empresa INNER JOIN webpsi_criticos.horarios h ON h.id=c.id_horario INNER JOIN 
                                          webpsi_criticos.zonales z ON z.id=c.id_zonal INNER JOIN webpsi_criticos.dias d ON d.id=c.id_dia INNER JOIN 
                                          webpsi_criticos.motivos m ON m.id=c.id_motivo INNER JOIN webpsi_criticos.submotivos s ON s.id=c.id_submotivo INNER JOIN
                                          webpsi_criticos.estados es ON es.id=c.id_estado INNER JOIN webpsi.tb_usuario t ON c.id_usuario=t.id left join 
                                          webpsi_criticos.liquidados l on c.id_gestion=l.id_gestion
                          )AS averias 
           UNION ALL
                          SELECT * FROM( 
                                          SELECT cri.id_atc,p.eecc_final,
                                                             (CASE c.fecha_movimiento WHEN (SELECT MAX(mov2.fecha_movimiento) FROM webpsi_criticos.gestion_movimientos mov2
                                                                                         WHERE mov2.id_gestion=c.id_gestion) THEN  'X' ELSE '' END) 'ultimo_movimiento',tipo_actividad,p.codigo_req AS 'averia',p.tipo_motivo as 'codmotivo_req_catv',p.quiebre,nombre_cliente_critico,telefono_cliente_critico,
                                          celular_cliente_critico,c.observacion, c.fecha_agenda,horario,d.dias, e.nombre,z.zonal,m.motivo,s.submotivo,es.estado,tecnico,
                                          p.fecha_Reg AS 'fecha_registro',fecha_creacion, fecha_movimiento,t.usuario,es.id AS 'e_id',c.fecha_consolidacion,
                                          origen AS 'tipo_averia',aver_m1 AS 'averia_m1',
                                          l.penalizable,
                                          (SELECT fecha_cambio FROM webpsi_coc.`tmp_provision_historico` pch WHERE pch.codigo_req=p.codigo_req ) AS fecha_cambio
                                          FROM
                                          webpsi_criticos.gestion_provision p INNER JOIN webpsi_criticos.gestion_criticos cri ON p.id_gestion=cri.id INNER JOIN 
                                          webpsi_criticos.gestion_movimientos c ON c.id_gestion=cri.id INNER JOIN 
                                          webpsi_criticos.empresa e ON e.id=c.id_empresa INNER JOIN webpsi_criticos.horarios h ON h.id=c.id_horario INNER JOIN 
                                          webpsi_criticos.zonales z ON z.id=c.id_zonal INNER JOIN webpsi_criticos.dias d ON d.id=c.id_dia INNER JOIN 
                                          webpsi_criticos.motivos m ON m.id=c.id_motivo INNER JOIN webpsi_criticos.submotivos s ON s.id=c.id_submotivo INNER JOIN
           ...
           UNION ALL
                          SELECT * FROM( 
                                          ...
                                          '' AS fecha_cambio
                                          FROM
                                          webpsi_criticos.gestion_rutina_manual a INNER JOIN webpsi_criticos.gestion_criticos cri ON a.id_gestion=cri.id INNER JOIN 
                                          webpsi_criticos.gestion_movimientos c ON c.id_gestion=cri.id INNER JOIN 
                                          webpsi_criticos.empresa e ON e.id=c.id_empresa INNER JOIN webpsi_criticos.horarios h ON h.id=c.id_horario INNER JOIN 
                                          webpsi_criticos.zonales z ON z.id=c.id_zonal INNER JOIN webpsi_criticos.dias d ON d.id=c.id_dia INNER JOIN 
                                          webpsi_criticos.motivos m ON m.id=c.id_motivo INNER JOIN webpsi_criticos.submotivos s ON s.id=c.id_submotivo INNER JOIN
           ...
           )AS t1  $filtro ORDER BY fecha_registro asc";
