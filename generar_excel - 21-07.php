...
if($filtro_inicial=="1"){
	...
	if($actividad !=""){
			...
			$pos = "";
			if(($alta!==false && $rutina!==false) || ($alta!==false && $rutina===false)){
			...
			}else if($alta===false && $rutina!==false){
				$pos = $rutina - 1;
				$actuacion = substr($actividad ,$pos,strlen($actividad));
				if($filtro_sql!=""){
					$filtro_sql .= " and actuacion in(".$comodin_averias.$actuacion.")";
			...
			if($actividad =="'Averias'"){//si solo marcan averias
				$comodin_prov = "";
			}
	...
	$data = array("actividad"=>$actividad,"empresa"=>$empresa,"zonal"=>$zonales,"negocio"=>$negocio,"microzona"=>$microzona,"area2"=>$area2,"lejano"=>$lejano,
		...
		"movistar1"=>$movistar1,"fecha_ini"=>$fecha_ini,"fecha_fin"=>$fecha_fin);
...
$table = "<table><tr><td>Averia</td><td>atc</td><td>Tipo Actividad</td><td>nombre</td><td>Fecha Registro</td><td>quiebres</td><td>empresa</td><td>telefono_codclientecms</td>
				 ...
				 <td>fono1</td><td>telefono</td><td>MDF</td><td>Microzona</td><td>observacion_102</td><td>segmento</td><td>area_</td><td>direccion_instalacion</td><td>codigo_distrito</td>
		...
		foreach($cliente as $row){
            ...
            $table .= "<tr><td>".$row["averia"]."</td><td>".$row["id_atc"]."</td><td>".$row["tipo_actividad"]."</td><td>".$row["nombre"]."</td>".
					  ...
					  "<td>".$row["segmento"]."</td><td>".$row["area_"]."</td><td>".$row["direccion_instalacion"]."</td><td>".$row["codigo_distrito"]."</td>".
